...
We can also define the point $X_A$. The distance from the pink line can be calculated as $W^TX$ divided by the L2 norm $\vert\vert W \vert\vert$.
...
In general binary classification can go to either positive or negative class: $\texttt{sign}(\rho) \in \{-1, 1\}$.

To calculate the L2 norm you use:

$\vert\vert {W} \vert\vert = \sqrt{\sum_{i=0}^d w_i^2}$

where $d$ is the dimension of the hyperplane, in here just 2, and for $w_0$ represents the bias.
