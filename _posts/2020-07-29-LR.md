...
We can also define the point $X_A$. The distance from the pink line can be calculated as $W^TX$ divided by the L2 norm $\|W \|_2$.
...
$\|W \|_2 = \sqrt{\sum_{i=0}^d w_i^2}$, where $d$ is the dimension of the hyperplane, in here just 2, and for $w_0$ represents the bias.
