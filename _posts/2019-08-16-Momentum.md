...
$$ p_i = p_{i-1} - lr * grad_{i-i} $$

In here the parameter $p_{i}$ represents the tensor of the batch step $i$.
...
We do calculate the gradients on backward pass for every mini batch. A mini batch is a package of items. This may be the package of images, or audios, or text, or gene sequence. 
...
It would be nice to predict the moving trends for our gradients taking in account every mini-batch so far. This may involve analyzing the gradients mean for every parameter at each step $i$ where we actually try to do is to find the *average moving gradient*.
...
But, this kind of mean doesn't promptly reflect actual data.
