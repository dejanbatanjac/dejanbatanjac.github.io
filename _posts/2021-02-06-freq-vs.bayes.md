...
In other words for a Bayesian person the **experiments are not random**. There is always a **cause**.
...
Bayesian methods work for **any** number of
data points $\vert X \vert$.
 
> $\vert X \vert$ stands for **cardinality** of $X$
 
Frequentists work **only** when the number of data points is much bigger than the number of parameters $\vert X \vert \gg \vert \theta\vert$.
 
Again, for neural networks Bayesian approach suits better since we may have millions of parameters and just a thousand data points.
...
_Frequentists simply don't have such tools._
