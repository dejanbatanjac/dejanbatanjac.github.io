...
**BertForSequenceClassification** is a special model based on the **BertModel** with the linear layer where you can set **self.num_labels** to number of classes you predict.
...
Let we in here just demonstrate `BertForMaskedLM` predicting words with high probability from the BERT dictionary based on a [MASK].
