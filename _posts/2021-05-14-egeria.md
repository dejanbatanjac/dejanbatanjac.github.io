...
Egeria is the world's first open source metadata standard grown from the needs of IBM, SAS, Cloudera, ING Bank and a few other big data governance companies.
...
Referenceable means this is an element that has a unique name. For instance any `Asset` entity type is under `Referenceable` entity type viewed from the Inheritance view.
