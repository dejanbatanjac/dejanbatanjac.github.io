...
The volume we get is called **activation map** or *feature map*.

The particular dot products from the feature map are called the features.

You can understand this dot products or features as if the filter _likes_ the specific area of the input image. 

The feature value may be positive, negative or zero.
...
## Calculate the size of the activation maps

Let's try to solve the base question. For the input image of volume 224x224x3 we 64 filters of size 5x5.
...
*Note: This formula doesn't yet use the dilated convolution in which case we have:*
