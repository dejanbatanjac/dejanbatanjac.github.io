...
Variational Auto-Encoders do **variational inference** thus the name and this is an **approximate** inference type.
...
Posterior distribution $p(\boldsymbol z \mid \boldsymbol  x)$ is the distribution of the latent variable conditioned on the data. This distribution is **continuous**.
...
The **sampler** is capable of creating samples from the Gaussian distribution with $\mathbb E(\boldsymbol{z})$ and $\mathbb V(\boldsymbol{z})$, but we use the reparameterization trick which is to replace the sampler with the equation:
 
$$\boldsymbol z = \mathbb{E}(\boldsymbol{z}) + \boldsymbol{\epsilon} \cdot \sqrt{\mathbb{V}(\boldsymbol{z})} \tag{1}$$
