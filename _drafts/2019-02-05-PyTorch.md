...
You can use all major Python packages like scipy, numpy, matplotlib and Cython with PyTorch's own autograd system.
...
Probable the next thing would be to convert your numpy arrays to PyTorch
...
~~~
a = torch.range(1, 12)
a = a.view(3, 4)        # reshapes into 3 columns x 4 rows
~~~
...
To execute a backpropagation algorithm you need to have a loss function in order to calculate the error. You can create your own loss function or use one from the list:
